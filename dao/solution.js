...
solution.findListWithCost = function(aid,next){
    const sql = `SELECT SUM(l.money) as money,SUM(l.service_charge) as service, SUM(l.shows) as shows,SUM(l.click) as click, s.id, s.solution_name, s.advertiser_id, s.start_date , s.end_date ,s.budget  ,s.status ,s.disabled 
