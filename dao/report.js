...
report.findByHour =  function(aid, date, next){
  ...
  var sql = 'select sum(`ns`) as show_all, sum(`nc`) as click_all, sum(`money`) as money_all, hour(`logtime`) as hour '+
...
report.findByDay = function(aid, start, end, next){
  ...
  // 调试无数据，去掉advertiserid filter，上线需恢复
  var sql = 'select sum(`ns`) as show_all, sum(`nc`) as click_all, sum(`money`) as money_all, date(`logtime`) as day '+
            'from traffic_consume_realtime WHERE '+
            // 'advertiserid = '+ aid +' AND '+
